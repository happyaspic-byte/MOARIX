  ...
  await tx.query(
    `INSERT INTO inventory_movements
       (id, company_id, warehouse_id, item_id, movement_type, quantity, unit_cost,
        reference_type, reference_id, reference_number, reason, idempotency_key, created_by)
    ...
    [
      ...
      input.referenceNumber ?? `MAN-${id.slice(0, 8).toUpperCase()}`,
      input.reason,
      input.idempotencyKey,
      session.userId,
    ],
...
export function postInventoryMovement(session: SessionContext, input: InventoryMovementInput) {
  return withCompany(session.companyId, (tx) =>

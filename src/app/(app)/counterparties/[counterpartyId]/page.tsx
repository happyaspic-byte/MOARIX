...
export default async function Customer360Page({ params }: { params: Promise<{ counterpartyId: string }> }) {
  ...
  return <>
    ...
    <section className="card case-section">
      ...
      {sites.length === 0 ? <EmptyState title="등록된 사업장이 없습니다." /> : <div className="table-wrap"><table className="data-table"><caption className="sr-only">고객 사업장</caption><thead><tr><th>사업장·코드</th><th>주소</th><th>고객 담당자</th><th>SI업체 담당자</th><th>시간대</th><th>운영 자산</th><th>진행 케이스</th>{canEditSite ? <th>관리</th> : null}</tr></thead><tbody>{sites.map((site) => <tr key={site.id}><td><div className="table-title"><strong>{site.name}</strong><small>{site.code} · {site.is_active ? "운영" : "비활성"}</small></div></td><td>{site.address ?? "—"}</td><td><div className="table-title"><strong>{site.contact_name ?? "—"}</strong><small>{[site.contact_phone, site.contact_email].filter(Boolean).join(" · ") || "연락처 미등록"}</small></div></td><td><div className="table-title"><strong>{site.si_contact_name ?? "—"}</strong><small>{[site.si_contact_phone, site.si_contact_email].filter(Boolean).join(" · ") || "연락처 미등록"}</small></div></td><td>{site.timezone}</td><td>{site.asset_count}대</td><td>{site.open_case_count}건</td>{canEditSite ? <td><div className="row-actions"><details className="create-panel"><summary className="button small">수정</summary><div className="create-drawer"><div className="drawer-head"><div><h2>{site.name} 수정</h2><p>사업장 위치와 고객·SI업체 담당자를 갱신합니다.</p></div><DrawerCloseButton /></div><SiteForm counterparties={counterparties} initial={{ id: site.id, counterparty_id: customer.id, counterparty_name: customer.name, code: site.code, name: site.name, address: site.address, contact_name: site.contact_name, contact_phone: site.contact_phone, contact_email: site.contact_email, si_contact_name: site.si_contact_name, si_contact_phone: site.si_contact_phone, si_contact_email: site.si_contact_email, timezone: site.timezone, asset_count: String(site.asset_count), open_case_count: String(site.open_case_count) }} /></div></details>{site.is_active ? <DeleteSiteForm id={site.id} name={site.name} /> : null}</div></td> : null}</tr>)}</tbody></table></div>}
    </section>

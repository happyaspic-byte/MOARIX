...
export default async function SettlementsPage() {
  ...
  const [invoices, bills, settlements, counterparties] = await Promise.all([
    listOpenDocuments(session.companyId, "invoice"),
    listOpenDocuments(session.companyId, "bill"),
  ...
  return <>
    ...
    <div className="metric-grid">
      <article className="metric-card"><span>미수</span><strong>{formatMoney(invoices.reduce((sum, row) => sum + Number(row.open_amount), 0), "KRW")}</strong></article>
      <article className="metric-card"><span>미지급</span><strong>{formatMoney(bills.reduce((sum, row) => sum + Number(row.open_amount), 0), "KRW")}</strong></article>
    ...
    <section className="card"><header className="card-header"><div><h2>미결 에이징</h2></div></header>
      {invoices.concat(bills).length === 0 ? <EmptyState title="미결 문서 없음" description="확정된 청구가 모두 배부되었거나 아직 없습니다." /> : <table className="data-table"><thead><tr><th>구분</th><th>번호</th><th>거래처</th><th>만기</th><th>미결</th><th>에이징</th></tr></thead><tbody>{[...invoices, ...bills].map((row) => <tr key={row.id}><td>{row.kind === "invoice" ? "미수" : "미지급"}</td><td>{row.number}</td><td>{row.counterparty_name}</td><td>{row.due_date ?? "—"}</td><td>{formatMoney(row.grand_total, "KRW")}</td><td>{agingLabels[row.aging]}</td></tr>)}</tbody></table>}
    </section>

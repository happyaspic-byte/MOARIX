...
export function InspectionForm({ assets, defaultAssetId }: { assets: AssetRow[]; defaultAssetId?: string }) {
  const [state, action] = useActionState(createInspectionAction, initialFormState);
  const initialAsset = assets.find((asset) => asset.id === defaultAssetId && asset.site_id && asset.status !== "retired");
  ...
  return <form ref={formRef} action={action} className="form-grid">
    <label className="full"><span>점검 자산 *</span><select name="assetId" required defaultValue={initialAsset?.id ?? ""}><option value="" disabled>고객·사업장·자산 선택</option>{assets.filter((asset) => asset.site_id && asset.status !== "retired").map((asset) => <option key={asset.id} value={asset.id}>{asset.counterparty_name} · {asset.site} · {asset.vendor_asset_id ?? asset.asset_tag}</option>)}</select></label>

...
type SearchParams = Promise<{ q?: SearchValue; status?: SearchValue; severity?: SearchValue; sla?: SearchValue }>;
...
export default async function ServicePage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <>
    <PageHeader eyebrow="SERVICE OPERATIONS" title="서비스 케이스" description="고객 문의와 장애를 자산에 연결하고 심각도, SLA 처리 기한, 다음 조치일로 우선순위를 관리합니다." actions={canWrite ? <details className="create-panel"><summary className="button primary"><Plus size={17} />케이스 접수</summary><div className="create-drawer"><div className="drawer-head"><div><h2>서비스 케이스 접수</h2><p>선택한 고객의 운영 자산, 연락처, 지원 권한과 외부 케이스를 함께 연결합니다.</p></div><DrawerCloseButton /></div><ServiceCaseForm counterparties={counterparties} assets={assets} /></div></details> : undefined} />
    ...
    <form className="filter-bar" action="/service" method="get" aria-label="서비스 케이스 필터">
      ...
      {(search || status || severity || slaFilter) ? <Link className="button" href="/service">초기화</Link> : null}
    </form>

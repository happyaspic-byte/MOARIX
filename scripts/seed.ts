...
async function seed() {
  ...
  await withCompany(companyId, async (tx) => {
    ...
    await tx.query(
      `INSERT INTO assets
         (id, company_id, counterparty_id, asset_tag, product_name, serial_number, status, site, installed_at, warranty_until, support_until, notes)
       VALUES ($1, $2, $3, 'AST-0001', '산업용 이중화 어플라이언스', 'DEMO-SN-001', 'active', '창원 1공장', CURRENT_DATE - 120, CURRENT_DATE + 245, CURRENT_DATE + 245, '분기 점검 대상')`,
      [assetId, companyId, customerId],
    );
    await tx.query(
      `INSERT INTO service_cases
         (id, company_id, number, counterparty_id, asset_id, title, description, severity, status, assigned_to, due_at, created_by)
       VALUES ($1, $2, 'CS-2026-0001', $3, $4, '정기 점검 일정 확인', '3분기 예방 점검 일정을 고객과 협의합니다.', 'normal', 'open', $5, now() + interval '14 days', $5)`,

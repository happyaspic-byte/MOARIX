...
| 자산·서비스 | 설치 장비와 지원 케이스 | 회사·고객 참조 무결성 |
| 보고서 | 업무 집계 | 원장 데이터에서 재계산 |
| 감사 | 누가 무엇을 변경했는지 기록 | UPDATE/DELETE 금지 |

## 멀티테넌시

업무 테이블은 `company_id`를 복합 외래키와 인덱스에 포함합니다. 서비스는 `withCompany()` 트랜잭션에서 `app.current_company_id`를 설정하며, PostgreSQL에서는 `FORCE ROW LEVEL SECURITY` 정책이 다른 회사 행을 차단합니다.
